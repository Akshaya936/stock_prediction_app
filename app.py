...
if symbol:
    ...
    if df.empty:
        st.error("❌ No data found. Please try a different stock.")
        st.stop()
    ...
    ax.plot(
        range(len(train_plot), len(train_plot) + len(actual_plot)),
        actual_plot,
        label='Actual'
    )
    ax.plot(
        range(len(train_plot), len(train_plot) + len(predicted_plot)),
        predicted_plot,
        label='Predicted'
    )
